...
@EventBusSubscriber
public class BreakingChemistryModBiomes {
	@SubscribeEvent
		...
		for (LevelStem levelStem : levelStemTypeRegistry.stream().toList()) {
			...
			if (dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					addParameterPoint(parameters, new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(-0.5f, 2f), Climate.Parameter.span(-1f, 1f), Climate.Parameter.span(0.3f, 1f), Climate.Parameter.span(-0.6f, 0.6f),
							Climate.Parameter.span(0.2f, 0.9f), Climate.Parameter.span(-1f, 1f), 0), biomeRegistry.getHolderOrThrow(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("breaking_chemistry", "sulfurbiome")))));
					chunkGenerator.biomeSource = MultiNoiseBiomeSource.createFromList(new Climate.ParameterList<>(parameters));
					chunkGenerator.featuresPerStep = Suppliers
							.memoize(() -> FeatureSorter.buildFeaturesPerStep(List.copyOf(chunkGenerator.biomeSource.possibleBiomes()), biome -> chunkGenerator.generationSettingsGetter.apply(biome).features(), true));
				}
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					((BreakingChemistryModNoiseGeneratorSettings) (Object) noiseGenerator.settings.value()).setbreaking_chemistryDimensionTypeReference(dimensionType);
				}
			}
		}
	}
	...
	private static SurfaceRules.RuleSource injectOverworldSurfaceRules(SurfaceRules.RuleSource currentRuleSource) {
		...
		customSurfaceRules.add(anySurfaceRule(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("breaking_chemistry", "sulfurbiome")), Blocks.SAND.defaultBlockState(), Blocks.RED_SAND.defaultBlockState(),
				BreakingChemistryModBlocks.SULFURORE.get().defaultBlockState()));
	...
	public interface BreakingChemistryModNoiseGeneratorSettings {
		void setbreaking_chemistryDimensionTypeReference(Holder<DimensionType> dimensionType);
	}
}