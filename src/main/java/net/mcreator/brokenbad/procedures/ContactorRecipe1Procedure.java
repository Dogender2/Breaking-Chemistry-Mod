...
public class ContactorRecipe1Procedure {
		...
		if (getEnergyStored(world, BlockPos.containing(x, y, z), null) >= 2000) {
			if (getAmountInGUISlot(entity, 2) < 1) {
				if ((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof BreakingChemistryModMenus.MenuAccessor _menu2 ? _menu2.getSlots().get(0).getItem() : ItemStack.EMPTY)
						.getItem() == BreakingChemistryModItems.FILTERED_ACID_SOLUTION.get()) {
					if ((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof BreakingChemistryModMenus.MenuAccessor _menu4 ? _menu4.getSlots().get(1).getItem() : ItemStack.EMPTY)
							.getItem() == BreakingChemistryModItems.HYDROGENCHLORIDE_IN_TANK.get()) {
						if (entity instanceof Player _player && _player.containerMenu instanceof BreakingChemistryModMenus.MenuAccessor _menu) {
							...
							ItemStack _setstack8 = new ItemStack(BreakingChemistryModItems.WET_BLUE_CRYSTALS.get()).copy();
